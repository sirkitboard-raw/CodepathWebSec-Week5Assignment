    ...
    <table>
      ...
      <?php while($message = db_fetch_assoc($message_result)) { ?>
        <?php
          ...
          $sender = mysqli_fetch_object(find_agent_by_id($message['sender_id']));
          $agent = mysqli_fetch_object(find_agent_by_id($message['recipient_id']));
          ...
          if(verify_signature($message['cipher_text'], $message['signature'], $sender->public_key)) {
            ...
            $validity_text = "DONT TRUST THIS MESSAGE";
          }
        ?>
        <tr>
