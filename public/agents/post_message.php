<?php
  ...
  if(isset($_POST['submit'])) {
    ...
    $sender = mysqli_fetch_object(find_agent_by_id(1));
    $agent = mysqli_fetch_object(find_agent_by_id($_GET['id']));

    $encrypted_text = pkey_encrypt($_POST['plain_text'], $agent->public_key);

    $signature = create_signature($_POST['plain_text'], $sender->private_key);
